...
def update_ops_manual():
    ...
    add_table(doc,
        ...
        [
            ["ci.yml", "Push / PR to main", "Lint · dbt parse · security scan. No BigQuery needed."],
            ["dbt-ci.yml", "PR touching dbt_project/**", "dbt compile + slim build (state:modified+)"],
            ["pipeline-daily.yml", "Cron 06:00 UTC / workflow_dispatch", "Full ELT: DQ → dbt staging → marts → integration tests → Slack"],
            ["deploy-dashboard.yml", "Push to dashboard/** / workflow_dispatch", "Deploy dashboard/index.html to GitHub Pages"],
        ]
    )
    ...
    add_para(doc, "5.  If the job uses environment: production or bigquery-ci and requires approval, click Review deployments → Approve and deploy.")

    add_heading(doc, "How to Trigger the Daily Pipeline Manually", level=2)
    add_para(doc, "Actions tab → Daily Pipeline — Full Olist ELT → Run workflow dropdown → set full_pipeline:")
    add_para(doc, "  false (default) — runs dbt staging + marts + integration tests only (data already in BigQuery).")
    add_para(doc, "  true — downloads CSVs from GCS, runs Python DQ validation, Meltano ELT, then dbt.")
    ...
    add_table(doc,
        ["Name", "Type", "Value"],
        [
            ["GCP_SERVICE_ACCOUNT_KEY", "Secret", "Full JSON of olist-analytics-gcp-key.json"],
    ...
    add_table(doc,
        ...
        [
            ["bigquery-ci", "dbt-ci.yml", "Gates GCP_SERVICE_ACCOUNT_KEY access on PRs"],
            ["production", "pipeline-daily.yml", "Gates production BigQuery access on daily runs"],
            ["github-pages", "deploy-dashboard.yml", "Auto-created on first successful Pages deploy"],
        ]
    )
...
def update_tdd():
    ...
    add_table(doc,
        ...
        [
            ...
            ["Scheduled data pipeline", "pipeline-daily.yml — backup/fallback", "Primary — 06:00 UTC, asset graph, retries"],
            ["Pipeline observability", "Flat pass/fail logs only", "Rich UI — run history, retries, asset lineage"],
            ["Dashboard deployment", "deploy-dashboard.yml — GitHub Pages", "—"],
            ["Manual pipeline trigger", "workflow_dispatch", "Materialise all in Dagster UI"],
        ]
    )
    ...
    add_para(doc, "First PR ever: no prior manifest → falls back to full staging build → uploads first manifest to GCS.")
    add_para(doc, "Subsequent PRs: downloads prior manifest → runs state:modified+ (changed models only) → uploads updated manifest.")

    add_heading(doc, "Concurrency Strategy", level=2)
    add_table(doc,
        ["Workflow", "cancel-in-progress", "Reason"],
        [
            ["ci.yml", "true (feature branches), false (main)", "New push supersedes old check on same branch; main validates every commit"],
            ["dbt-ci.yml", "true", "New PR commit invalidates previous CI run entirely"],
            ["pipeline-daily.yml", "false", "Never cancel mid-run — partial BigQuery state is worse than a queued run"],
            ["deploy-dashboard.yml", "true", "Newer deploy supersedes older — no partial state risk with static files"],
        ]
    )

    add_heading(doc, "Key Implementation Patterns", level=2)
    add_para(doc, "1. dbt deps before every dbt command — dbt_packages/ is gitignored, packages must be installed on every runner.")
    add_para(doc, "2. setup-gcloud@v2 after auth@v2 — required for gsutil to use authenticated credentials.")
    add_para(doc, "3. always() on downstream jobs — prevents transitive skip propagation through conditional job chains.")
    add_para(doc, "4. GCS bucket IAM — service account needs objectAdmin role granted once: gsutil iam ch ...")
    add_para(doc, "5. Variables vs Secrets — non-sensitive config (GCP_PROJECT_ID, GCS_BUCKET_NAME) goes in Variables tab, not Secrets.")
...
def update_srs():
    ...
    add_table(doc,
        ...
        [
            ["CI-01", "Every push to any branch must trigger automated Python linting and formatting checks", "ci.yml — ruff"],
            ["CI-02", "Every push must trigger a dbt SQL syntax validation without requiring BigQuery credentials", "ci.yml — dbt parse"],
            ["CI-03", "Every push must trigger a Python security scan", "ci.yml — bandit"],
            ["CI-04", "Every PR touching dbt_project/** must validate SQL against live BigQuery", "dbt-ci.yml — dbt compile"],
            ["CI-05", "PRs must only rebuild and test the changed dbt models and their downstream dependents", "dbt-ci.yml — state:modified+"],
            ["CI-06", "The full ELT pipeline must run automatically at 06:00 UTC daily as a fallback", "pipeline-daily.yml — cron"],
            ["CI-07", "The pipeline must be manually triggerable with a full re-ingestion option", "pipeline-daily.yml — workflow_dispatch"],
            ["CI-08", "Dashboard updates must be automatically deployed to GitHub Pages on merge to main", "deploy-dashboard.yml"],
            ["CI-09", "Pipeline failures must send a Slack notification", "pipeline-daily.yml — Slack webhook"],
        ]
    )
    ...
    add_table(doc,
        ...
        [
            ["NFR-CI-01", "No credentials may appear in YAML workflow files", "google-github-actions/auth@v2 + GitHub Secrets"],
            ["NFR-CI-02", "ci.yml must require no BigQuery credentials", "dbt parse uses dummy credentials; no real connection"],
            ["NFR-CI-03", "Running pipeline must never be cancelled mid-run", "concurrency: cancel-in-progress: false on pipeline-daily.yml"],
            ["NFR-CI-04", "CI/CD must not replace Dagster scheduling", "pipeline-daily.yml is a fallback; Dagster remains primary"],
        ]
    )
